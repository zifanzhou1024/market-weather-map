...
Writes to public/data/candidates/tradingview_move_candidate.json only.
...
Symbol confirmed by live probe 2026-05-11 against rongardF/tvdatafeed@e6f6aaa7de43:
  TVC:MOVE — daily OHLCV from 2005-12-15 to present, ~5000 bars.

Usage (CI only — local run without secrets skips gracefully):
  python -m scripts.ingest.fetch_tradingview_move
"""
...
def _write_payload(rows: list[dict]) -> None:
    payload = {
        ...
        "notes": (
            ...
            "Not treated as the official ICE MOVE feed. Not active in scoring."
        ),
        "observations": rows,
    }
