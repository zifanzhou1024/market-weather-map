"""Tests for scripts/ingest/fetch_tradingview_put_call.py.
...
from scripts.ingest import fetch_tradingview_put_call as mod
...
def test_writes_candidate_file_on_success(
    ...
    """main() writes tradingview_put_call_candidate.json with correct shape."""
    ...
    out = tmp_path / "candidates" / "tradingview_put_call_candidate.json"
    assert out.exists(), "Expected candidate JSON not written"

    payload = json.loads(out.read_text())
    assert payload["series_id"] == "tradingview_put_call_candidate"
    ...
    assert payload["observations"][0] == {"date": "2024-01-01", "value": 0.78}
    assert payload["observations"][1] == {"date": "2024-01-02", "value": 0.82}
