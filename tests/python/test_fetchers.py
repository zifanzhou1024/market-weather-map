...
from scripts.ingest.fetch_cboe import normalize_vix_rows
...
def test_catalog_uses_fed_credit_series_instead_of_restricted_oas_sources():
    ...
    assert "high_yield_oas" not in catalog_text
    assert "investment_grade_oas" not in catalog_text
    assert "BAMLH0A0HYM2" not in catalog_text
    assert "BAMLC0A0CM" not in catalog_text
...
def test_normalize_vix_rows_requires_date_and_close_columns():
    with pytest.raises(ValueError, match="missing required VIX columns"):
        normalize_vix_rows([{"timestamp": "2026-04-30", "last": "16.5"}])
